...
class Server::Tracker::GalileoTest < ActiveSupport::TestCase
  ...
  def validate_status_props(packet)
    props = %w{ status sens_moving bad_angle int_power_low
      ...
      engine_on sens_hit glonass signal alarm_mode alarm }
  ...
  test "parse packet speed tag" do
    packet = @server.parse_packet("\x33\x5C\x00\x48\x08").first
  ...
  test "parse packet status1 tag" do
    packet = @server.parse_packet("\x40\x01\x00").first
    validate_status_props(packet)
  ...
  test "parse packet status2 tag" do
    packet = @server.parse_packet("\x40\x02\x00").first
    validate_status_props(packet)
  ...
  test "parse packet status3 tag" do
    packet = @server.parse_packet("\x40\x20\x00").first
    validate_status_props(packet)
  ...
  test "parse packet status4 tag" do
    packet = @server.parse_packet("\x40\x40\x00").first
    validate_status_props(packet)
  ...
  test "parse packet status5 tag" do
    packet = @server.parse_packet("\x40\x80\x00").first
    validate_status_props(packet)
  ...
  test "parse packet status6 tag" do
    packet = @server.parse_packet("\x40\x00\x01").first
    validate_status_props(packet)
  ...
  test "parse packet status7 tag" do
    packet = @server.parse_packet("\x40\x00\x02").first
    validate_status_props(packet)
    ...
    assert_equal(false, packet[:int_bus_power_low])
    assert_equal(false, packet[:ext_power_low])
    assert_equal(true, packet[:engine_on])
    assert_equal(false, packet[:sens_hit])
    assert_equal(false, packet[:glonass])
    assert_equal(0, packet[:signal])
    assert_equal(false, packet[:alarm_mode])
  ...
  test "parse packet status8 tag" do
    packet = @server.parse_packet("\x40\x00\x04").first
    validate_status_props(packet)
  ...
  test "parse packet status9 tag" do
    packet = @server.parse_packet("\x40\x00\x08").first
    validate_status_props(packet)
  ...
  test "parse packet status10 tag" do
    packet = @server.parse_packet("\x40\x00\x30").first
    validate_status_props(packet)
  ...
  test "parse packet status11 tag" do
    packet = @server.parse_packet("\x40\x00\x40").first
    validate_status_props(packet)
  ...
  test "parse packet status12 tag" do
    packet = @server.parse_packet("\x40\x00\x80").first
    validate_status_props(packet)
  ...
  test "parse packet power_input tags" do
    0.upto(3).each do |tag_index|
      packet = @server.parse_packet((0x50 + tag_index).chr + "\x5A\x5A").first
  ...
  test "parse packet thermometers tags" do
    1.upto(7).each do |tag_index|
      packet = @server.parse_packet((0x70 + tag_index).chr + "\x01\xD8").first
  ...
  test "parse packet fms_distance tag" do
    packet = @server.parse_packet("\xC2\x01\x02\x03\x04").first
    assert(packet.key? :fms_distance)
    assert_equal(336529925, packet[:fms_distance])
  end

  test "parse packet can_b1 tag" do
    packet = @server.parse_packet("\xC3\x01\x02\x03\x04").first
  ...
  test "parse packet can8bitr tags" do
    0.upto(17).each do |tag_index|
      packet = @server.parse_packet((0xC4 + tag_index).chr + "\xA5").first
  ...
  test "parse packet can16bitr tags" do
    0.upto(4).each do |tag_index|
      packet = @server.parse_packet((0xD6 + tag_index).chr + "\x5A\x5A").first
  ...
  test "parse packet can32bitr tags" do
    0.upto(4).each do |tag_index|
      packet = @server.parse_packet((0xDB + tag_index).chr + "\x01\x02\x03\x04").first
  ...
  test "several packets in one data pack" do
    packets = @server.parse_packet("\x10\x01\x00\x10\x02\x00")
    assert_equal(2, packets.count)
    assert_equal(1, packets.first[:record_id])
    assert_equal(2, packets.last[:record_id])
  end
